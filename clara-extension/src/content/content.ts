...
// Automatic Legal Document Detection Heuristic
function detectLegalDocument(): boolean {
...
function extractSecurityEvidence() {
  ...
  let storageCount = 0;
  try {
    storageCount = window.localStorage ? window.localStorage.length : 0;
  } catch (e) {
    storageCount = 0;
  }
  ...
  return {
    ...
    hasPasswordInput,
    hasCreditCardInput,
    documentCookieLength: document.cookie ? document.cookie.length : 0,
    protocol: window.location.protocol,
    hostname: window.location.hostname,
    storageCount,
    isLegalDoc: detectLegalDocument()
  };
}
...
new MutationObserver(() => {
  ...
  if (url !== lastUrl) {
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime) {
      chrome.runtime.sendMessage({ type: 'CLARA_PAGE_NAVIGATED', url, title: document.title });
    }
  }
}).observe(document, {subtree: true, childList: true});

// Render ambient legal audit notification banner if legal doc detected
if (detectLegalDocument()) {
  ...
  `;
  banner.onclick = () => {
    // Request sidebar toggle via background script
    if (typeof chrome !== 'undefined' && chrome.runtime) {
...
// Bi-directional Clause Highlighting Logic
function highlightClauseText(clauseText: string): boolean {
  ...
  // Extract snippet query (first 60 chars)
  ...
  // Fallback fuzzy search if exact text snippet not matched
...
// Listen for window postMessages (for standalone web app preview)
